...
export let insetPanelWebview: vscode.Webview | undefined;
export function getPanelContent(
  panel: vscode.WebviewEditorInset,
  page: string | undefined = undefined,
  edits: FileEdit[] | undefined = undefined,
  isFullScreen: boolean = false
): string {
  insetPanelWebview = panel.webview;
  panel.onDidDispose(() => {
    insetPanelWebview = undefined;
  });

  let extensionUri = getExtensionUri();
  let scriptUri: string;
  let styleMainUri: string;
  let vscMediaUrl: string = insetPanelWebview
    .asWebviewUri(vscode.Uri.joinPath(extensionUri, "gui"))
    .toString();

  const isProduction = true; // context?.extensionMode === vscode.ExtensionMode.Development;
  if (isProduction) {
    scriptUri = insetPanelWebview
      .asWebviewUri(vscode.Uri.joinPath(extensionUri, "gui/assets/index.js"))
      .toString();
    styleMainUri = insetPanelWebview // Test same style file 
...
//   resolveWebviewView(
//     webviewView: vscode.WebviewEditorInset,
//     _context: vscode.WebviewViewResolveContext,
//     _token: vscode.CancellationToken
//   ): void | Thenable<void> {
//     webviewView.webview.html = getPanelContent(webviewView);
//   }
// }
